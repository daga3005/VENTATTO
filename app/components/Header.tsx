...
export default function TattooStudioSite() {
  ...
  const links = [
    ...
    { id: 2, name: "Galería", href: '/galery' },
    { id: 3, name: "Contacto", href: '/contacto' },
    { id: 4, name: "Sobre mi", href: '/about_me' },
    { id: 5, name: "Reseñas", href: '/resenas' },
  ];
  ...
  return (
    <>
      <header className={`fixed w-full z-50 transition-all duration-500 ${
        scrolled 
          ? "bg-neutral-900/95 backdrop-blur-md border-b border-orange-600/30 shadow-2xl" 
          : "bg-transparent"
      }`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-20">
            ...
            {/* Mobile Menu Button */}
            <button
              className="md:hidden text-red-400 cursor-pointer z-50 relative"
      ...
      {/* Mobile Menu */}
      <div className={`md:hidden fixed inset-y-0 right-0 w-full max-w-sm bg-neutral-900 z-40 transition-transform duration-500 shadow-2xl ${
        ...
        <div className="flex flex-col items-center justify-center h-full gap-6 px-4">
          {links.map((item) => (
            <Link
              key={item.id}
              href={item.href}
              className="text-2xl text-gray-300 hover:text-yellow-400 transition-colors py-2"
              onClick={() => setMenuOpen(false)}
            >
              {item.name}
            </Link>
          ...
          <Link 
            href={socialNetworks[3].url}
            className="px-8 py-3 rounded-full bg-gradient-to-r from-orange-600 to-red-600 text-white font-bold hover:shadow-lg hover:shadow-orange-600/50 transition-all duration-300 mt-4"
            onClick={() => setMenuOpen(false)}
          >
            Reservar Cita
          </Link>
